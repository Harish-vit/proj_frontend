...
const Profile = ({ user, setLoginUser }) => {
  ...
  const handleEditName = async () => {
    try {
      const response = await axios.put(`http://localhost:4000/user/editname`, { email: user.email, name });
  ...
  const handleEditPassword = async () => {
    try {
      const response = await axios.put(`http://localhost:4000/user/editpassword`, { email: user.email, password });
      console.log(response);
  ...
  const handleAddSelectedGenre = async () => {
    try {
      const response = await axios.put(`http://localhost:4000/user/addgenre`, { email: user.email, genre: selectedGenre });
      console.log(response);
  ...
  const handleDeleteGenre = async (genre) => {
    try {
      const response = await axios.put(`http://localhost:4000/user/deletegenre`, { email: user.email, genre });
      console.log(response);
